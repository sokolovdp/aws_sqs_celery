...
class SQSClientExtended(object):
    ...
    def receive_message(self, queue_url, max_number_Of_Messages=1, wait_time_seconds=10):
        ...
        for message in opt_messages:
            large_pay_load_attribute_value = message.get('MessageAttributes', {}).get(
            ...
            if large_pay_load_attribute_value:
                try:
                    ...
                    if 's3BucketName' not in message_body and 's3Key' not in message_body:
                        raise ValueError(
                            'Detected missing required key attribute s3BucketName and s3Key in s3 payload')
    ...
    def send_message(self, queue_url, message, message_attributes={}):
        ...
        if self.always_through_s3 or self.__is_large(str(message), message_attributes):
            ...
            return self.sqs.send_message(QueueUrl=queue_url, MessageBody=s3_key_message,
    ...
    def __store_message_in_s3(self, message_body):
        ...
        try:
            ...
            print(
                "Failed to store the message content in an S3 object. SQS message was not sent. {}, type:{}".format(
                    str(e), type(e).__name__))
            raise e
...
if __name__ == '__main__':
    ...
    from .config import QUEUE_URL, ACCESS_KEY, SECRET_KEY, REGION

    sqs = SQSClientExtended(ACCESS_KEY, SECRET_KEY, REGION, 'tiptapcode-sqs-data')
